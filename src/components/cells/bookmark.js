...
export default class Bookmark extends React.Component{
  ...
  reader_link_html = (html) => {
    return html.replace(
      /<p class="post_archived_links">([\s\S]*?)<a href="([^"]+)"[^>]*>([\s\S]*?)<\/a>([\s\S]*?)<\/p>/g,
      '<p><a class="post_archived_links" href="$2">$1$3$4</a></p>'
    )
  }
  
  render_html = () => {
    const { bookmark } = this.props
    const html = this.reader_link_html(bookmark.content_html)
